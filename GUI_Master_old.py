...
root.title("Banana Leaf Detection")
...
#
lbl = tk.Label(root, text="Banana Leaf Detection", font=('times', 35,' bold '), width=65,height=1,bg="Red3",fg="white")
lbl.place(x=0, y=0)
...
def test_model_proc(fn):
    ...
    if fn!="":
        ...
        if yoga == 0:
            Cd="Healthy \n Pesticides used :Chlorpyrifos , Thiabendazole"
            
        elif yoga == 1:
            Cd="black-sigatoka \n Pesticides used :Timorex Gold"
            
        elif yoga == 2:
            Cd="panama-disease \n Pesticides used : Foliar spray of Copper Oxychloride \n Thiophanate Methyl  "
            
        elif yoga == 3:
            Cd="yellow-sigatoka \n Pesticides used : copper oxychloride, mancozeb, \n chlorothalonil or carbendazim"
...
def openimage():
    ...
    global fn
    print(fn)
    fileName = askopenfilename(initialdir='D:/banana Leaf disease detection/banana Leaf disease detection', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
